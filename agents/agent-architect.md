...
2. **File structure follows `batuta-agent-authoring`'s template** (frontmatter + Role / When to invoke / When NOT to invoke / Output format / Examples). On top of that template, every specialist this agent creates MUST also include:
   ...
   - **Research-first Step 2 (conditional, mandatory for code-writing specialists).** If the specialist's `tools` declare `Edit`, `Write`, or `MultiEdit`, its `## Workflow` MUST include an explicit Step 2 that mirrors `agents/implementer.md` Step 2 (post-v2.5) verbatim: for every external library, framework, API, or service the slice imports, calls, or upgrades, run Context7 lookup (`mcp__context7__resolve-library-id` then `mcp__context7__query-docs`) at the pinned version; on no-coverage, web-search the official docs domain or GitHub repo; add a citation comment at every import site you touch using language-appropriate syntax (`# Source: <url> (verified YYYY-MM-DD, <lib>@<version>)` for Python/YAML/Shell, `// Source: ...` for JS/TS/Rust/Go/Java/C/C++, `-- Source: ...` for SQL). Skip this bullet if the specialist is read-only (Read/Grep/Glob only).
   - **Audit-scope Step 0 (conditional, mandatory for audit-gate specialists).** If (and only if) the specialist closes with `AUDIT RESULT: APPROVED` or `AUDIT RESULT: BLOCKED` as its final literal line — i.e. it is a domain-specific audit gate (e.g. a `dian-fe-validator` that audits Colombian e-invoicing compliance) — its `## Workflow` MUST include a Step 0 pre-flight mirroring `agents/code-reviewer.md` Step 0 (post-v2.5): run `git diff --staged --stat` and `git diff HEAD --stat`; if both report no changes, return `AUDIT RESULT: NOT APPLICABLE — no code diff to audit. The audit chain runs only after an implementation slice produces changes; during exploration, planning, ad-hoc queries, or spec-writing the chain does not apply.` and stop. Skip this bullet for code-writing specialists (the vast majority).
   - A `## Workflow` section whose final step writes the build-log to the canonical project-local path: `docs/plans/active/<slice-id>/build-log.md` (preferred, current convention) OR `specs/current/<slice-id>/build-log.md` (legacy SDD layout, only if `docs/plans/active/` does not exist in the consumer project). NEVER write `build-log.md` to project root. The Workflow's final line is the literal `READY FOR AUDIT: test-engineer → code-reviewer → security-auditor` (for code-writing specialists) or `AUDIT RESULT: APPROVED|BLOCKED|NOT APPLICABLE` (for audit-gate specialists).
   - An `## Absolute rules` section that always includes "NO modifications to `specs/` or `docs/plans/` except `build-log.md`", "NO new libraries without Step 2 research-first verification (Context7 lookup + citation comment)", and "On a domain BLOCKER, stop and return `BLOCKER: <description>`", plus any domain-specific rules
...
3. **Programmatic checks before closing the file** (these are enforced in addition to the rules from `batuta-agent-authoring`):
   ...
   - Workflow ends with the literal closing line `READY FOR AUDIT: test-engineer → code-reviewer → security-auditor` (code-writing specialists) or `AUDIT RESULT: APPROVED|BLOCKED|NOT APPLICABLE` (audit-gate specialists)
   - **Run `batuta-agent-authoring` verification rules 5 and 6 against the generated file** (post-v2.5 — see `skills/batuta-agent-authoring/SKILL.md`). Rule 5: if the agent declares `Edit`/`Write`/`MultiEdit` in `tools`, the body MUST include the research-first Step 2 from sub-section 2 above. Rule 6: if the agent closes with `AUDIT RESULT:` literal, the body MUST include the Step 0 NOT-APPLICABLE pre-flight from sub-section 2 above. If either rule fails, edit the generated specialist before declaring it ready.
   - Build-log path is `docs/plans/active/<slice-id>/build-log.md` (preferred) OR `specs/current/<slice-id>/build-log.md` (legacy fallback). Reject if the body hardcodes `specs/current/` as the only option or writes to project root.
